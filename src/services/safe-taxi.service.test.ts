...
describe("SafeTaxiService.settleRide — CARD", () => {
  ...
  it("odrzuca rozliczenie bez pokrycia i nie kredytuje kierowcy ani platformy", async () => {
    const rideRow = {
      id: "ride_card_1",
      passengerId: "user_pass",
      driverId: "user_driver",
      ...
      wallet: {
        findUnique: vi.fn().mockImplementation((args: { where: { userId: string } }) => {
          if (args.where.userId === "user_pass") {
            return Promise.resolve({ id: "w_pass", balance: 500n });
          }
          if (args.where.userId === "user_driver") {
    ...
    expect(tx.wallet.update).not.toHaveBeenCalledWith(
      expect.objectContaining({ where: { userId: "user_driver" } }),
    );
    expect(tx.wallet.update).not.toHaveBeenCalledWith(
      expect.objectContaining({ where: { userId: "user_platform" } }),
    );
