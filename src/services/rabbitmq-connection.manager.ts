...
export class RabbitMqConnectionManager extends EventEmitter {
  ...
  async waitForReady(timeoutMs = 60_000): Promise<void> {
    ...
    throw new Error(
      `RabbitMQ: timeout oczekiwania na połączenie (${timeoutMs}ms)`,
    );
  }
  ...
  async publish(routingKey: string, payload: unknown): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      const timeout = setTimeout(() => {
        reject(
          new Error(
            `RabbitMQ: timeout potwierdzenia publikacji (${this.publishConfirmTimeoutMs}ms)`,
          ),
        );
      }, this.publishConfirmTimeoutMs);
  ...
  private scheduleReconnect(): void {
    ...
    const exp = Math.min(
      this.initialBackoffMs * Math.pow(2, Math.min(this.attempt, 24)),
      this.maxBackoffMs,
    );
    this.attempt += 1;

    console.warn(
      `[RabbitMQ] ponowne połączenie za ${Math.round(exp)}ms (próba ${this.attempt})`,
    );
