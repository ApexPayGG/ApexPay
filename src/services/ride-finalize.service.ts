  ...
  async finalizeRide(input: RideFinalizeInput, req?: Request): Promise<RideFinalizeResult> {
    ...
    return this.prisma.$transaction(
      async (tx) => {
        ...
        const callerIntegratorId = req?.user?.id?.trim();
        if (
          callerIntegratorId === undefined ||
          connectedAccount.integratorUserId !== callerIntegratorId ||
          connectedAccount.userId !== ride.driverId
        ) {
          throw new RideFinalizeAuthorizationError("Subkonto kierowcy nie należy do tego przejazdu.");
        }
        ...
        if (passengerDebit.count !== 1) {
          throw new RideFinalizeInsufficientFundsError(
            "Brak środków pasażera na rozliczenie przejazdu.",
          );
        }
