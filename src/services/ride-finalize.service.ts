...
export class RideFinalizeUnauthorizedError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "RideFinalizeUnauthorizedError";
  }
}
    ...
    return this.prisma.$transaction(
      async (tx) => {
        ...
        if (ride.paymentMethod !== RidePaymentMethod.CARD) {
          throw new RideFinalizeInvalidStateError("Przejazd nie jest płatnością kartową.");
        }
        if (ride.status !== SafeTaxiRideStatus.CREATED) {
          throw new RideFinalizeInvalidStateError("Przejazd nie oczekuje na finalizację.");
        }
        ...
        if (
          ...
          throw new RideFinalizeUnauthorizedError("Subkonto kierowcy nie jest powiązane z przejazdem.");
        }
        ...
        const passengerChargeAmount = baseAmount + tipAmount;
        const passengerDebit = await tx.wallet.updateMany({
          where: {
            id: passengerWallet.id,
            balance: { gte: passengerChargeAmount },
          },
          data: { balance: { decrement: passengerChargeAmount } },
        });
