...
export class RideFinalizeAuthorizationError extends Error {
  constructor(message = "Integrator is not allowed to finalize this ride.") {
    super(message);
...
export class RideFinalizeInvalidStateError extends Error {
  constructor(message = "Ride cannot be finalized in its current state.") {
    super(message);
      ...
      async (tx) => {
        ...
        if (
          ...
          throw new RideFinalizeInvalidStateError(
            "Przejazd nie oczekuje na rozliczenie kartowe.",
          );
        }
        ...
        if (
          ...
          throw new RideFinalizeAuthorizationError();
        }
        ...
        const passengerDebit = baseAmount + tipAmount;

        const debited = await debitWalletByIdIfFunded(tx, passengerWallet.id, passengerDebit);
        if (!debited) {
