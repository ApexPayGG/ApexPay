import type { FraudCheck, Prisma } from "@prisma/client";
import {
  AuditAction,
  AuditActorType,
  FraudCheckStatus,
...
import { ALL_FRAUD_RULES, type FraudContext } from "../lib/fraud-rules.js";
...
export class FraudBlockedError extends Error {
  constructor(
    public readonly fraudCheckId: string,
    public readonly score: number,
    message = "Operacja zablokowana ze względów bezpieczeństwa (fraud).",
  ) {
    super(message);
...
export type FraudCheckResult = {
  ...
  score: number;
  rulesTriggered: Array<{ rule: string; score: number; detail: string }>;
  fraudCheckId: string;
};
...
export class FraudDetectionService {
   ...
   */
  async evaluate(context: FraudContext): Promise<FraudCheckResult> {
    const readPrisma = context.prisma;
    const ruleCtx: FraudContext = { ...context, prisma: readPrisma };
    ...
    const rulesJson: Prisma.InputJsonValue = triggered.map((t) => ({
      rule: t.rule,
      score: t.score,
      detail: t.detail,
    }));
  ...
  async reviewFraudCheck(
    fraudCheckId: string,
    adminUserId: string,
    decision: "APPROVE" | "CONFIRM_FRAUD",
  ): Promise<FraudCheck> {
  ...
  async listForAdmin(
    filters: FraudListFilters,
    limit: number,
    cursorEncoded: string | undefined,
  ): Promise<{ items: FraudCheck[]; nextCursor: string | null }> {
    ...
    const parts: Prisma.FraudCheckWhereInput[] = [];
  ...
  async getById(id: string): Promise<FraudCheck | null> {
    return this.prisma.fraudCheck.findUnique({ where: { id } });
  }

  /** FLAGGED, utworzone w ostatniej godzinie, bez przeglądu — monitoring. */
  async countUnreviewedFlaggedRecent(hoursBack: number): Promise<number> {
