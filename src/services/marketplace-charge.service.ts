...
export function mergeSplitLines(splits: SplitLine[]): Map<string, bigint> {
...
export function mergeIntegrationSplitLines(
  splits: SplitLine[],
): Map<string, bigint> {
  if (splits.length === 0) {
    return new Map();
...
function parseMarketplaceConnectedCreditRef(referenceId: string): {
  chargeId: string;
  connectedAccountId: string;
} | null {
  const marker = ":credit:";
...
async function loadConnectedAccountIdsByChargeId(
  prisma: PrismaClient,
  chargeIds: string[],
...
export class MarketplaceChargeService {
  ...
  async createIntegrationCharge(params: {
  ...
  }): Promise<{ charge: MarketplaceCharge }> {
    ...
    if (splitSum > params.amountCents) {
      ...
      throw new MarketplaceValidationError(
        `Suma splits (${splitSum}) nie może przekraczać amount (${params.amountCents}).`,
      );
    }
    const platformFeeCents = params.amountCents - splitSum;

    const paymentMethodIdTrimmed =
      params.paymentMethodId !== undefined && params.paymentMethodId.trim().length > 0
    ...
    if (accountIds.length > 0) {
      ...
      for (const a of accounts) {
        ...
        if (a.userId === null) {
          await params.redis.del(redisKey);
          throw new MarketplaceValidationError(
            "Subkonto musi być powiązane z użytkownikiem (KYC / portfel), aby otrzymać split.",
          );
        }
    ...
    let fraudEval:
      | Awaited<ReturnType<FraudDetectionService["evaluate"]>>
      | undefined;
    ...
    try {
      ...
      if (this.webhookPublish !== undefined) {
        void this.webhookPublish(txResult.webhookOutboxId).catch((err: unknown) => {
  ...
  async createConnectedAccount(userId: string): Promise<{ id: string }> {
  ...
  async setConnectedAccountStatus(
    accountId: string,
  ...
  async chargeSplit(params: {
    ...
    if (sum !== amountCents) {
      throw new MarketplaceValidationError(
        `Suma splits (${sum}) musi równać się amountCents (${amountCents}).`,
      );
    }
    ...
    const idToUser = new Map(
      accounts.map((a) => [a.id, a.userId] as [string, string]),
    );
  ...
  async listForIntegration(
    ...
    const items: IntegrationChargeListItem[] = slice.items.map((r) => ({
      id: r.id,
