...
export class PayoutService {
  ...
  async settlePayout(
    ...
    outcome: "PAID" | "FAILED",
    pspReferenceId?: string | null,
    audit?: { request?: Request | undefined; adminUserId?: string | undefined },
  ): Promise<Payout> {
    ...
    const pspTrimmed =
      pspReferenceId !== undefined && pspReferenceId !== null
        ? pspReferenceId.trim()
        : undefined;
    const pspForDb =
      pspTrimmed === undefined
        ? undefined
        : pspTrimmed.length > 0
        ...
        if (outcome === "PAID") {
          ...
          if (userId === null) {
            throw new MarketplaceValidationError(
              "Subkonto nie ma powiązanego portfela — nie można zwrócić środków.",
            );
          }
    ...
    if (this.webhookPublish !== undefined) {
      void this.webhookPublish(settled.webhookOutboxId).catch((err: unknown) => {
  ...
  async createPayout(params: {
    ...
    if (account.userId === null) {
      await params.redis.del(redisKey);
      throw new MarketplaceValidationError(
        "Subkonto musi być powiązane z użytkownikiem (portfel), aby zlecić wypłatę.",
      );
    }
    ...
    const currency =
      (params.currency !== undefined ? params.currency.trim() : "PLN").toUpperCase() ||
      "PLN";
  ...
  async listForIntegration(
    ...
    const items: IntegrationPayoutListItem[] = slice.items.map((r) => ({
      id: r.id,
      amount: r.amount,
      currency: r.currency,
      status: r.status,
