...
export class IntegrationsPayoutController {
  ...
  async listPayouts(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const { items: rows, nextCursor } = await this.payoutService.listForIntegration(
        integratorUserId,
  ...
  async exportPayouts(req: Request, res: Response): Promise<void> {
    const integratorUserId = req.user?.id?.trim();
    if (integratorUserId === undefined || integratorUserId.length === 0) {
      res.status(401).json({ error: "Unauthorized", code: "UNAUTHORIZED" });
      return;
    ...
    try {
      ...
      const csv = toCsv(
        ["ID", "Kwota (PLN)", "Waluta", "Subkonto ID", "Status", "PSP Reference ID", "Data utworzenia"],
        filtered.map((row) => [
          row.id,
