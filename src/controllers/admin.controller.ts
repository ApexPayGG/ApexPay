...
import { AuditAction, type AuditLog, type PrismaClient } from "@prisma/client";
...
function isAuditAction(s: string): s is AuditAction {
  return Object.values(AuditAction).includes(s as AuditAction);
}
...
export class AdminController {
  ...
  async listAuditLogs(req: Request, res: Response): Promise<void> {
    try {
      ...
      const actorId =
        typeof actorIdRaw === "string" && actorIdRaw.trim().length > 0
          ? actorIdRaw.trim()
          : undefined;

      const entityTypeRaw = req.query.entityType;
      const entityType =
        typeof entityTypeRaw === "string" && entityTypeRaw.trim().length > 0
      ...
      const filters: Parameters<AuditLogService["listForAdmin"]>[0] = {};
      ...
      const { items, nextCursor } = await this.auditLogService.listForAdmin(
        filters,
        limit,
        cursor,
      );
  ...
  async listTransactions(req: Request, res: Response): Promise<void> {
    try {
      ...
      const prefixRaw =
        typeof req.query.referenceIdPrefix === "string"
          ? req.query.referenceIdPrefix
          : "";
      const referenceIdPrefix =
        prefixRaw.trim().length > 0 ? prefixRaw.trim().slice(0, 128) : undefined;

      const { items, total } = await this.walletService.listTransactionsAdmin(
        skip,
        limit,
        referenceIdPrefix !== undefined ? { referenceIdPrefix } : undefined,
      );
      res.status(200).json({
        items,
        page,
  ...
  async listUsersWithWallets(req: Request, res: Response): Promise<void> {
    try {
      const limitRaw = Number.parseInt(String(req.query.limit ?? "50"), 10);
      const limit = Number.isFinite(limitRaw)
  ...
  async settlePayout(req: Request, res: Response): Promise<void> {
    const payoutId =
    ...
    try {
      ...
      const payout = await this.payoutService.settlePayout(
        payoutId,
        body.status,
        body.pspReferenceId,
        {
          request: req,
          adminUserId:
            adminUserId !== undefined && adminUserId.length > 0 ? adminUserId : undefined,
        },
      );
