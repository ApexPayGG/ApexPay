...
describe("MatchController.resolveDispute", () => {
  ...
  it("returns 409 and skips payout when a legacy resolve targets a non-disputed match", async () => {
    h.matchFindUnique.mockResolvedValue({
      id: "m1",
      tournamentId: "t1",
      status: "PENDING",
  ...
  it("returns 409 and skips payout when a legacy resolve targets a v1-settled match", async () => {
    h.matchFindUnique.mockResolvedValue({
      id: "m1",
      tournamentId: "t1",
      status: "SETTLED",
