...
} from "../services/webhook-dead-letter.service.js";

function parseRequeuedFilter(raw: unknown): boolean | undefined {
...
export class WebhookDeadLetterAdminController {
  constructor(private readonly service: WebhookDeadLetterService) {}
  ...
  async list(req: Request, res: Response): Promise<void> {
    try {
      ...
      const integratorUserId =
        typeof integratorRaw === "string" && integratorRaw.trim().length > 0
          ? integratorRaw.trim()
          : undefined;

      const requeuedRaw = req.query["requeued"];
      let requeued: boolean | undefined;
