...
export class AutopayItnWebhookController {
  ...
  async handle(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (itn.PaymentStatus === "SUCCESS") {
        ...
        if (setOk !== "OK") {
          const hasDurableDeposit = await this.walletService.hasPspDeposit(itn.RemoteID);
          if (hasDurableDeposit) {
            await this.markDone(idempKey);
            res.status(200).type("application/xml").send(confirmationXml(itn.ServiceID, itn.OrderID));
            return;
          }
          contextLogger().info(
            { orderId: itn.OrderID, remoteId: itn.RemoteID },
            "Autopay ITN duplicate while processing",
          );
  ...
  private async markDone(idempKey: string): Promise<void> {
    try {
      await this.redis.set(idempKey, IDEMP_DONE, "EX", IDEMP_TTL_SEC);
    } catch (redisErr) {
      contextLogger().warn(
        { err: redisErr instanceof Error ? redisErr.message : String(redisErr) },
        "Autopay ITN idempotency done write failed",
      );
    }
  }
}
