...
import {
  FraudCheckNotFoundError,
  FraudDetectionService,
} from "../services/fraud-detection.service.js";
...
function isFraudCheckStatus(s: string): s is FraudCheckStatus {
  return Object.values(FraudCheckStatus).includes(s as FraudCheckStatus);
}
...
export class FraudAdminController {
  ...
  async list(req: Request, res: Response): Promise<void> {
    try {
      ...
      let status: FraudCheckStatus | undefined;
      ...
      const filters: Parameters<FraudDetectionService["listForAdmin"]>[0] = {};
      if (status !== undefined) {
      ...
      const { items, nextCursor } = await this.fraudDetectionService.listForAdmin(
        filters,
        limit,
        cursor,
      );
  ...
  async review(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const updated = await this.fraudDetectionService.reviewFraudCheck(
        id,
        adminUserId,
        body.decision,
      );
