...
import { ConnectedAccountSubjectType, type ConnectedAccountStatus } from "@prisma/client";
import { z, ZodError } from "zod";
import {
  ConnectedAccountDuplicateError,
  ConnectedAccountService,
} from "../services/connected-account.service.js";
...
export class IntegrationsAccountController {
  constructor(private readonly service: ConnectedAccountService) {}
  ...
  async list(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const { items: rows, nextCursor } = await this.service.listForIntegration(
        integratorUserId,
  ...
  async create(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const row = await this.service.createForIntegration(
        integratorUserId,
        {
          email: body.email,
          subjectType,
