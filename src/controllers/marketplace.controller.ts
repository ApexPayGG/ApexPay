...
export class MarketplaceController {
  constructor(private readonly service: MarketplaceChargeService) {}
  ...
  async patchConnectedAccount(req: Request, res: Response): Promise<void> {
    try {
      ...
      const allowed: ConnectedAccountStatus[] = [
  ...
  async createCharge(req: Request, res: Response): Promise<void> {
    try {
      ...
      const debitUserId =
        typeof body.debitUserId === "string" ? body.debitUserId.trim() : "";
      ...
      for (const row of body.splits) {
        ...
        const r = row as { connectedAccountId?: unknown; amountCents?: unknown };
        const cid =
