...
describe("POST /internal/webhooks/autopay-itn", () => {
  ...
  it("PENDING nie blokuje późniejszego SUCCESS dla tego samego OrderID+RemoteID", async () => {
    process.env.AUTOPAY_SHARED_KEY = "testkey123";
    process.env.AUTOPAY_SERVICE_ID = "123456";
    process.env.AUTOPAY_GATEWAY_URL = "https://pay-accept.bm.pl";
    process.env.AUTOPAY_RETURN_URL = "https://app.example.com/payments/return";
    ...
    const pending = buildItnBase64({ ...common, status: "PENDING" });
    const success = buildItnBase64({ ...common, status: "SUCCESS" });
    ...
    expect(pendingRes.text).toContain("<confirmation>CONFIRMED</confirmation>");
