...
const STATIC_MATCH_ID =
  process.env.STRESS_MATCH_ID ?? "stress-idempotency-match-static";
const STATIC_IDEMPOTENCY_KEY =
  process.env.STRESS_IDEMPOTENCY_KEY ?? "stress-idempotency-key-static-v1";
const BASE_URL = (process.env.STRESS_BASE_URL ?? "http://localhost:3000").replace(
  /\/$/,
  "",
);
const FINAL_WINNER_ID =
  process.env.STRESS_FINAL_WINNER_ID ?? "stress-winner-static";
...
async function main(): Promise<void> {
  ...
  if (jwt === undefined || jwt.length === 0) {
    console.error(
      "[stress] Ustaw STRESS_JWT (Bearer token użytkownika z uprawnieniami).",
    );
    process.exit(1);
  }
  ...
  if (!skipDb) {
    if (databaseUrl === undefined || databaseUrl.length === 0) {
      console.error(
        "[stress] Ustaw DATABASE_URL (lub STRESS_SKIP_DB_VERIFY=1 aby pominąć weryfikację Outbox).",
      );
      process.exit(1);
    }
  ...
  console.info(
    `[stress] ${PARALLEL} równoległych POST, matchId=${STATIC_MATCH_ID}, Idempotency-Key=${STATIC_IDEMPOTENCY_KEY}`,
  );
  ...
  console.info("");
  console.info(
    `[stress] Podsumowanie HTTP: 200=${ok200}, 409=${conflict409}, inne kody=${otherHttp}, błąd sieci=${netErr}`,
  );
  ...
  if (pool !== null) {
    ...
    console.info(
      `[stress] Outbox FUNDS_SETTLED (matchId): przed=${outboxBefore}, po=${outboxAfter}, delta=${deltaOutbox}`,
    );
    await pool.end();
    if (deltaOutbox > 1) {
      console.error(
        "[stress] BŁĄD: więcej niż jedno rozliczenie w bazie (delta Outbox > 1).",
      );
      process.exit(1);
    }
  } else {
    console.warn(
      "[stress] Pominięto weryfikację bazy (STRESS_SKIP_DB_VERIFY=1).",
    );
  }
