...
describe("POST /api/v1/matches/:id/resolve (integration)", () => {
  ...
  it("rejects non-admin JWTs before legacy resolve reaches settlement", async () => {
    const redis = new FakeRedis() as unknown as import("ioredis").default;
    const wsService = {
      notifyWallet: vi.fn(),
    } as unknown as WebSocketService;
    const prismaTransaction = vi.fn().mockRejectedValue(new Error("should not settle"));
  ...
  it("rejects non-admin JWTs before v1 resolve reaches settlement", async () => {
    const redis = new FakeRedis() as unknown as import("ioredis").default;
    const wsService = {
      notifyWallet: vi.fn(),
    } as unknown as WebSocketService;
    ...
    const res = await request(app)
      ...
      .set("Authorization", `Bearer ${token(UserRole.PLAYER)}`)
      .set("Idempotency-Key", "non-admin-v1")
      .send({ finalWinnerId: "winner-1" });
  ...
  it("rejects non-admin JWTs before legacy wallet deposit mints balance", async () => {
    const redis = new FakeRedis() as unknown as import("ioredis").default;
    const wsService = {
      notifyWallet: vi.fn(),
    } as unknown as WebSocketService;
    ...
    const res = await request(app)
      ...
      .set("Authorization", `Bearer ${token(UserRole.PLAYER)}`)
      .send({ amount: "1000", referenceId: "manual-mint" });

    expect(res.status).toBe(403);
  });

  it("returns 400 when v1 settlement rejects a winner outside the match", async () => {
    const redis = new FakeRedis() as unknown as import("ioredis").default;
    const wsService = {
      notifyWallet: vi.fn(),
    } as unknown as WebSocketService;
    const rejectOutsider = vi
      .fn()
      .mockRejectedValue(new MatchSettlementError("WINNER_NOT_IN_MATCH"));

    const { app } = createApp({
      prisma: {} as PrismaClient,
      redis,
      wsService,
      matchSettlementService: { settleDisputedMatch: rejectOutsider },
    });

    const res = await request(app)
      .post("/api/v1/matches/match-race-1/resolve")
      .set("Authorization", `Bearer ${token()}`)
      .set("Idempotency-Key", "outsider-winner")
      .send({ finalWinnerId: "outsider" });
