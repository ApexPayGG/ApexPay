...
} from "./services/message-broker.js";
...
async function main(): Promise<void> {
  ...
  let webhookBroker: ApexpayWebhookRabbitMq | null = null;
  ...
  const webhookPublish =
    webhookBroker !== null
      ? (outboxId: string) => webhookBroker.publishOutboxDelivery(outboxId)
      : undefined;
  ...
  const disputeDeadlineTimer = setInterval(() => {
    void (async () => {
      try {
        const rows =
          await disputeServiceForAlerts.findDisputesWithEvidenceDeadlineWithinHours(48);
        for (const d of rows) {
        ...
        const deadLetterThreshold = Number(
          dlThresholdRaw !== undefined && dlThresholdRaw.trim() !== ""
            ? dlThresholdRaw
            : "10",
        );
        ...
        contextLogger().error(
          ...
          "Hourly operational alert tick failed (disputes / dead letters)",
        );
      }
    })();
  ...
  const fraudFlaggedTimer = setInterval(() => {
    void (async () => {
      try {
        ...
        if (n > 0) {
          contextLogger().warn(
            { unreviewedFlaggedCount: n },
            "Fraud: nierozpatrzone FLAGGED (ostatnia 1h)",
          );
        }
      } catch (e: unknown) {
        contextLogger().error(
          { err: e instanceof Error ? e.message : String(e) },
          "Fraud flagged alert tick failed",
        );
      }
    })();
  ...
  if (webhookBroker !== null) {
    ...
    void webhookDispatcher.startConsumer(webhookBroker).catch((e: unknown) => {
      const m = e instanceof Error ? e.message : String(e);
  ...
  httpServer.listen(PORT, () => {
    ...
    if (webUi !== undefined && webUi.length > 0) {
      console.log(
        `[ApexPay Web] UI statyczne: ${path.resolve(process.cwd(), webUi)} → http://localhost:${PORT}/`,
      );
    }
  });
