...
function attachUserFromJwtPayload(
  req: Request,
  ...
  const role =
    typeof payload.role === "string" && payload.role.length > 0 ? payload.role : undefined;
...
const MSG_NEED_AUTH =
  "Wymagane uwierzytelnienie: nagłówek x-api-key / Bearer apx_live_… albo sesja JWT.";
 ...
 */
export function createIntegratorHybridAuthMiddleware(
