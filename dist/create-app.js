...
export function createApp(options) {
    ...
    const corsAllowedOrigins = corsOriginEnv === undefined || corsOriginEnv.length === 0
        ? null
    ...
    const fraudDetectionService = new FraudDetectionService(options.prisma, auditLogService);
    const fraudAdminController = new FraudAdminController(fraudDetectionService);
    const disputeService = new DisputeService(options.prisma, options.redis, auditLogService, options.webhookPublish);
    ...
    const marketplaceChargeService = new MarketplaceChargeService(options.prisma, options.webhookPublish, auditLogService, fraudDetectionService);
    ...
    const integratorHybridAuthMiddleware = createIntegratorHybridAuthMiddleware(apiKeyService);
    const apiKeyController = new ApiKeyController(apiKeyService);
    const integrationsChargeController = new IntegrationsChargeController(marketplaceChargeService, options.redis);
    const integrationsPayoutController = new IntegrationsPayoutController(payoutService, options.redis);
    const refundService = new RefundService(options.prisma, auditLogService, options.webhookPublish);
    const integrationsRefundController = new IntegrationsRefundController(refundService, options.redis);
    const connectedAccountService = new ConnectedAccountService(options.prisma, auditLogService);
    const integrationsAccountController = new IntegrationsAccountController(connectedAccountService);
    const integratorConfigService = new IntegratorConfigService(options.prisma);
    const integrationsConfigController = new IntegrationsConfigController(integratorConfigService);
    ...
    const matchController = new MatchController(options.prisma, clearingService, wsService, bracketService);
    ...
    app.post("/internal/webhooks/psp-deposit", pspDepositWebhookHmac, (req, res) => {
        void pspDepositWebhookController.handle(req, res);
    });
    app.post("/internal/webhooks/psp-dispute", pspDepositWebhookHmac, (req, res) => {
        void pspDisputeWebhookController.handle(req, res);
    });
    app.post("/internal/webhooks/autopay-itn", express.urlencoded({ extended: true }), (req, res) => {
    ...
    app.use("/api/v1/admin", (req, res, next) => {
        ...
        next();
    }, authMiddleware, requireRole([UserRole.ADMIN]), adminRouter);
    app.use("/api/admin", authMiddleware, requireRole([UserRole.ADMIN]), adminRouter);
    app.post("/api/wallet/deposit", authMiddleware, requireRole([UserRole.ADMIN]), (req, res) => {
    ...
    app.post("/api/v1/connected-accounts", authMiddleware, requireRole([UserRole.ADMIN]), (req, res) => {
        void marketplaceController.createConnectedAccount(req, res);
    });
    app.patch("/api/v1/connected-accounts/:id", authMiddleware, requireRole([UserRole.ADMIN]), (req, res) => {
        void marketplaceController.patchConnectedAccount(req, res);
    });
    app.post("/api/v1/charges", authMiddleware, requireRole([UserRole.ADMIN]), (req, res) => {
    ...
    app.get("/api/v1/integrations/me", apiKeyAuthMiddleware, (req, res) => {
    ...
    app.get("/api/v1/integrations/charges/:chargeId/refunds", integratorHybridAuthMiddleware, (req, res) => {
        void integrationsRefundController.listForCharge(req, res);
    });
    app.post("/api/v1/integrations/charges/:chargeId/refunds", integratorHybridAuthMiddleware, (req, res) => {
    ...
    app.get("/api/v1/integrations/accounts", integratorHybridAuthMiddleware, (req, res) => {
        void integrationsAccountController.list(req, res);
    });
    app.post("/api/v1/integrations/accounts", integratorHybridAuthMiddleware, (req, res) => {
        void integrationsAccountController.create(req, res);
    });
    app.get("/api/v1/integrations/config", integratorHybridAuthMiddleware, (req, res) => {
        void integrationsConfigController.get(req, res);
    });
    app.put("/api/v1/integrations/config", integratorHybridAuthMiddleware, (req, res) => {
        void integrationsConfigController.put(req, res);
    });
    app.get("/api/tournaments", authMiddleware, (req, res) => {
        void tournamentController.listTournaments(req, res);
    });
    app.get("/api/tournaments/:id", authMiddleware, (req, res) => {
        void tournamentController.getTournament(req, res);
    });
    ...
    app.post("/api/tournaments/:id/start", authMiddleware, (req, res) => {
        void tournamentController.startTournament(req, res);
    });
    ...
    app.use((err, _req, res, _next) => {
        ...
        if (err instanceof Error) {
            log.error({ err: { message: err.message, stack: err.stack, name: err.name } }, "Unhandled error");
        }
        else {
