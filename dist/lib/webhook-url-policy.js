...
export function parseSafeWebhookUrl(rawUrl) {
    let url;
    try {
        url = new URL(rawUrl);
    }
    ...
    if (family !== 0 && !isPublicIpAddress(hostname, family)) {
        throw new UnsafeWebhookUrlError();
    }
    return url;
}
export function isSafeWebhookUrlForStorage(rawUrl) {
    try {
...
export const resolveWebhookHostname = async (hostname) => {
...
export async function assertWebhookUrlResolvesPublic(rawUrl, resolveHostname = resolveWebhookHostname) {
...
//# sourceMappingURL=webhook-url-policy.js.map