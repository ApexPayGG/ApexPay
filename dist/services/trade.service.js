...
import { Prisma, TradeStatus, TransactionType as TxType, } from "@prisma/client";
...
function platformUserIdFromEnv() {
    ...
    throw new TradePlatformConfigError("Brak APEXPAY_PLATFORM_USER_ID lub SAFE_TAXI_PLATFORM_USER_ID (prowizja trade).");
}
export class TradeService {
    ...
    async createTrade(sellerId, input) {
        if (input.amountCents <= 0) {
            throw new RangeError("amountCents must be positive");
        }
        const platformFeeCents = BigInt(Math.max(0, Math.round((input.amountCents * PLATFORM_FEE_PERCENT) / 100)));
    ...
    async listTradesForSeller(sellerId, options) {
    ...
    async payTrade(tradeId, buyerId) {
    ...
    async confirmReceipt(tradeId, buyerId) {
     ...
     */
    async cancelBySeller(tradeId, sellerId) {
        await this.prisma.$transaction(async (tx) => {
            const trade = await tx.trade.findUnique({ where: { id: tradeId } });
            if (trade === null) {
                throw new TradeNotFoundError();
