import { AuditAction, AuditActorType, DisputeReason, DisputeStatus, Prisma as PrismaNs, TransactionType as TxType, } from "@prisma/client";
import { z } from "zod";
...
const pspDisputeWebhookSchema = z
    ...
    .strict();
function toPositiveBigInt(amount) {
...
export class DisputeService {
    ...
    auditLogService;
    webhookPublish;
    constructor(prisma, redis, auditLogService, webhookPublish) {
        this.prisma = prisma;
        this.redis = redis;
        this.auditLogService = auditLogService;
        this.webhookPublish = webhookPublish;
    }
    parsePspWebhookBody(body) {
    ...
    async createFromWebhook(payload) {
        ...
        try {
            ...
            if (this.webhookPublish !== undefined) {
                void this.webhookPublish(result.webhookOutboxId).catch((err) => {
                    contextLogger().warn({ err: err instanceof Error ? err.message : String(err) }, "dispute.created: webhook publish failed");
                });
            }
    ...
    async submitEvidence(disputeId, evidence, adminUserId) {
    ...
    async resolve(disputeId, outcome, adminUserId) {
        const result = await this.prisma.$transaction(async (tx) => {
            ...
            const status = outcome === "LOST" ? DisputeStatus.LOST : DisputeStatus.ACCEPTED;
            const eventType = outcome === "LOST" ? "dispute.lost" : "dispute.accepted";
            const updated = await tx.dispute.update({
                where: { id: disputeId },
                data: {
                    status,
        ...
        if (this.webhookPublish !== undefined && result.webhookOutboxId !== null) {
            void this.webhookPublish(result.webhookOutboxId).catch((err) => {
                contextLogger().warn({ err: err instanceof Error ? err.message : String(err) }, "dispute resolved: webhook publish failed");
            });
        }
        return result;
    }
     ...
     */
    async findDisputesWithEvidenceDeadlineWithinHours(hours) {
        const now = new Date();
