...
import { Prisma, AuditAction, AuditActorType, RefundCoveredBy, RefundStatus, TransactionType as TxType, } from "@prisma/client";
 ...
 */
export async function loadChargeLedgerComposition(prisma, chargeId) {
    ...
    const connectedCredits = new Map();
    for (const r of rows) {
...
export function getMarketplacePlatformUserId() {
    ...
    throw new RefundConfigurationError("Ustaw APEXPAY_PLATFORM_USER_ID lub SAFE_TAXI_PLATFORM_USER_ID (portfel platformy do zwrotów).");
}
 ...
 */
export function allocateRefundCostSplit(refundAmount, chargeOriginal, platformCents, connectedCredits) {
    const perConnectedAccount = new Map();
 ...
 */
export function allocateRefundCostConnectedOnly(refundAmount, connectedCredits) {
    const out = new Map();
 ...
 */
export async function validateRefundEligibility(prisma, input) {
...
export class RefundService {
    ...
    async listForCharge(integratorUserId, chargeId) {
    ...
    async createRefund(params) {
        ...
        try {
            ...
            const out = await this.prisma.$transaction(async (tx) => {
                ...
                const applyDebit = async (walletId, amount, referenceId) => {
                    if (amount === 0n) {
                        return;
                    }
                    try {
                ...
                const applyDebitAllowNegative = async (walletId, amount, referenceId) => {
                    if (amount === 0n) {
                        return;
                    }
                    await tx.wallet.update({
                ...
                if (params.coveredBy === RefundCoveredBy.PLATFORM) {
                    await applyDebit(platformWallet.id, refundAmount, `ref:${refundId}:debit:platform`);
                }
                else if (params.coveredBy === RefundCoveredBy.CONNECTED_ACCOUNT) {
                    const perCa = allocateRefundCostConnectedOnly(refundAmount, composition.connectedCredits);
                    for (const [caId, cents] of perCa) {
                        ...
                        await applyDebitAllowNegative(w.id, cents, `ref:${refundId}:debit:ca:${caId}`);
                    }
                }
                else {
                    const { platformDebit, perConnectedAccount } = allocateRefundCostSplit(refundAmount, original, composition.platformCents, composition.connectedCredits);
                    if (platformDebit > 0n) {
                        await applyDebit(platformWallet.id, platformDebit, `ref:${refundId}:debit:platform`);
                    }
            ...
            if (this.webhookPublish !== undefined) {
                void this.webhookPublish(out.webhookOutboxId).catch((err) => {
