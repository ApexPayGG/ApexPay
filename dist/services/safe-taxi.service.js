import { Prisma, RidePaymentMethod, SafeTaxiRideStatus, TransactionType as TxType, } from "@prisma/client";
...
export function splitSafeTaxiFare(fareCents, commissionBps) {
...
export class SafeTaxiService {
    ...
    platformUserId() {
        ...
        if (raw === undefined || raw.length === 0) {
            throw new SafeTaxiConfigError("Brak SAFE_TAXI_PLATFORM_USER_ID — utwórz użytkownika-konto platformy i ustaw jego CUID w .env.");
        }
        return raw;
    }
    commissionBps() {
        ...
        if (!Number.isFinite(n) || n < 0 || n > 10_000) {
            throw new SafeTaxiConfigError("SAFE_TAXI_PLATFORM_COMMISSION_BPS musi być liczbą 0–10000 (np. 1500 = 15%).");
        }
    ...
    maxDriverDebtFloor() {
        ...
        if (debtRaw !== undefined && debtRaw.length > 0) {
            if (!/^-?\d+$/.test(debtRaw)) {
                throw new SafeTaxiConfigError("MAX_DRIVER_DEBT musi być liczbą całkowitą ≤ 0 (minor units, np. -10000 dla -100 PLN).");
            }
        ...
        if (legacy !== undefined && legacy.length > 0) {
            if (!/^\d+$/.test(legacy)) {
                throw new SafeTaxiConfigError("MAX_DRIVER_DEBT_MINOR_UNITS musi być nieujemną liczbą całkowitą (interpretacja: saldo ≥ -wartość).");
            }
            return -BigInt(legacy);
        }
        return null;
    }
    assertBalanceAfterCommissionOk(driverBalance, platformCut) {
    ...
    async assertDriverWithinDebtLimitForNewCashRide(driverId) {
        const floor = this.maxDriverDebtFloor();
        if (floor === null) {
            return;
        }
    ...
    async createRide(passengerId, driverId, paymentMethod = RidePaymentMethod.CARD) {
     ...
     */
    async settleRide(rideId, fareCents) {
        if (fareCents < 100n) {
        ...
        const { platformCents: platformCut, driverCents: driverCut } = splitSafeTaxiFare(fareCents, bps);
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            try {
                const debit = await tx.wallet.updateMany({
                    where: { id: passengerW.id, balance: { gte: fareCents } },
                ...
                if (err instanceof InsufficientFundsError || isInsufficientFundsDbError(err)) {
                    throw new InsufficientFundsError();
                }
                throw err;
            }
...
//# sourceMappingURL=safe-taxi.service.js.map