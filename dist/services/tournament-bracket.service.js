...
const TERMINAL = new Set(["RESOLVED", "SETTLED"]);
export class TournamentBracketService {
     ...
     */
    async advanceAfterTerminalMatch(tournamentId, tx) {
        await tx.$executeRaw(Prisma.sql `SELECT id FROM "Tournament" WHERE id = ${tournamentId} FOR UPDATE`);
        ...
        for (const R of rounds) {
            ...
            const ordered = [...inRound].sort((a, b) => a.createdAt.getTime() - b.createdAt.getTime());
