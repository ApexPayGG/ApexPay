...
function isInsufficientFundsDbError(err) {
...
export class TournamentController {
    ...
    async joinTournament(req, res) {
        try {
            ...
            const tournamentId = typeof rawTournamentId === "string"
                ? rawTournamentId
    ...
    async startTournament(req, res) {
        try {
            ...
            res.status(200).json({
                status: "success",
                message: "Turniej rozpoczęty. Utworzono mecze pierwszej rundy (status PENDING).",
                data: {
                    tournamentId: tid,
            ...
            switch (msg) {
                ...
                case "ODD_PLAYERS":
                    res.status(400).json({
                        error: "Liczba graczy musi być parzysta (MVP: brak wolnego losu). Dodaj lub usuń uczestnika.",
                    });
                    return;
                default:
                    res.status(500).json({
    ...
    async listTournaments(req, res) {
        try {
            ...
            if (limitParam !== undefined && limitParam !== "") {
                const raw = typeof limitParam === "string"
                    ? limitParam
            ...
            if (statusParam !== undefined && statusParam !== "") {
                const raw = typeof statusParam === "string"
                    ? statusParam
