...
export class PaymentsController {
    ...
    async rideFinalize(req, res) {
        ...
        let acquiredIdempotencyKey = null;
        try {
            ...
            const completed = await this.rideFinalizeService.findCompletedRide(finalizeInput);
            if (completed !== null) {
                res.status(200).json({
                    rideId: completed.rideId,
                    driverPayout: completed.driverPayout,
                    platformCommission: completed.platformCommission,
                    tip: completed.tip,
                    duplicate: true,
                });
                return;
            ...
            if (idemSet === null) {
                res.status(409).json({
                    error: "Rozliczenie przejazdu jest w toku albo nie zostało potwierdzone w bazie.",
                    code: "CONFLICT",
                });
                return;
            }
            ...
            if (acquiredIdempotencyKey !== null) {
                try {
            ...
            if (err instanceof InsufficientFundsError) {
                res.status(402).json({ error: "Niewystarczające środki u pasażera.", code: "PAYMENT_REQUIRED" });
                return;
            }
