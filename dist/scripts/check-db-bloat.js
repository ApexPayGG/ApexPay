...
async function main() {
  ...
  `;
    try {
        ...
        const row = rows[0];
        if (row === undefined) {
            console.warn('[db:check-bloat] Nie znaleziono tabeli "OutboxEvent" w schemacie public (brak migracji?).');
            return;
        }
        ...
        console.log(`[db:check-bloat] OutboxEvent — heap: ~${heapPretty} MiB, indeksy: ~${idxPretty} MiB`);
        if (heap === 0) {
            console.warn("[db:check-bloat] Heap tabeli = 0 — pusta tabela lub brak danych; wskaźnik indeks/heap niewiarygodny.");
            return;
        }
        const ratio = idx / heap;
        if (ratio > 2.5) {
            console.warn(`[db:check-bloat] OSTRZEŻENIE: suma indeksów (~${idxPretty} MiB) jest ~${ratio.toFixed(1)}× większa niż heap tabeli (~${heapPretty} MiB).`);
            console.warn("[db:check-bloat] Możliwy bloat / martwe krotki — rozważ: VACUUM ANALYZE \"OutboxEvent\"; w ciężkich przypadkach REINDEX na indeksach lub oknie serwisowym.");
        }
        else {
...
//# sourceMappingURL=check-db-bloat.js.map